...
plotting.indices <- list()
compact.hmm <- list()
cov.cnv.samples <- list()
window.positions <- list()
...
rm(temp.2R) 
# Get the vector of genomic positions contained in this region
window.positions$ace1 <- compact.hmm$ace1[[1]]$Position
window.positions$cyp6 <- compact.hmm$cyp6[[1]]$Position
# Get the indices within this region that we wish to use for plotting
plotting.indices$ace1 <- c(min(which(window.positions$ace1 >= region.coords$ace1[1])), max(which(window.positions$ace1 <= region.coords$ace1[2])))
plotting.indices$cyp6 <- c(min(which(window.positions$cyp6 >= region.coords$cyp6[1])), max(which(window.positions$cyp6 <= 28570000)))
...
rm(temp.3R) 
window.positions$cyp6mz <- compact.hmm$cyp6mz[[1]]$Position
window.positions$gste <- compact.hmm$gste[[1]]$Position
plotting.indices$cyp6mz <- c(min(which(window.positions$cyp6mz >= region.coords$cyp6mz[1])), max(which(window.positions$cyp6mz <= region.coords$cyp6mz[2])))
plotting.indices$gste <- c(min(which(window.positions$gste >= region.coords$gste[1])), max(which(window.positions$gste <= region.coords$gste[2])))
...
rm(temp.X) 
window.positions$cyp9k1 <- compact.hmm$cyp9k1[[1]]$Position
plotting.indices$cyp9k1 <- c(min(which(window.positions$cyp9k1 >= region.coords$cyp9k1[1])), max(which(window.positions$cyp9k1 <= region.coords$cyp9k1[2])))
...
known.cnvs <- list(ace1   = list(Dup1  = list(FA = matrix(c(3436850,3639550,3437150,3639850), 2, 2),
                   ...
                   cyp6   = list(Dup1  = list(FA = matrix(c(28480150, 28483300, 28480450, 28483600), 2, 2)),
                                 ...
                                 Dup15 = list(FA = matrix(c(15246250, 9676400, 15246550, 9676700), 2, 2),
                                             ...
                                             #                seq = c('GGGTT', 'CCCAA'))),
                                              BP = data.frame(pos = c(NA, 15246640),
                                                              seq = c(NA, 'CCCAA'))),
                                 Dup16 = list(FA = matrix(c(15222700, 15244450, 15223000, 15244750), 2, 2),
...
get.discordant.reads <- function(this.sample, target.folder, read.type, target.region.coords){
	read.type <- match.arg(read.type, c('FA', 'SS', 'FM', 'crosschrom'))
	...
	grep.string <- paste(read.type, '.*>= 10', sep = '')
	allpos <- pos.table[grepl(grep.string, pos.table$Type), c('Position', 'Mate.position')]
...
get.breakpoint.reads <- function(this.sample, target.folder, target.region.coords){
	...
	clipping.start.point <- pos.table[grepl('soft clipping start point mapq >= 10', pos.table$Type), c('Position', 'Clipped_sequence')]
	clipping.end.point <- pos.table[grepl('soft clipping end point mapq >= 10', pos.table$Type), c('Position', 'Clipped_sequence')]
	#
...
get.diagnostic.reads.allsamples <- function(sample.names, disc.target.folder, bp.target.folder, target.region.coords){
	output <- lapply(discordant.read.types, function(x) lapply(sample.names, get.discordant.reads, disc.target.folder, x, target.region.coords))
	output[['BP']] <- lapply(sample.names, get.breakpoint.reads, bp.target.folder, target.region.coords)
	output
}
...
count.diagnostic.reads <- function(sample.name, diagnostic.reads.list, known.cnvs.list, verbose = F){
	di <- lapply(diagnostic.reads.list, function(x) x[[sample.name]])
	num.reads <- lapply(known.cnvs.list, count.reads.per.dup, di)
	if (verbose)
		print(num.reads)
...
get.read.based.cnvs <- function(coverage.cnv.calls, diagnostic.read.counts.table, threshold.diagnostic.reads = 2){
	...
	cbind(Dup0 = rownames(read.based) %in% setdiff(coverage.cnv.calls, read.cnv.samples), read.based)
}
...
diagnostic.read.counts <- mcmapply(function(d, k) t(sapply(sample.names, count.diagnostic.reads, d, k)), diagnostic.reads, known.cnvs, mc.preschedule = F, mc.cores = num.cores)
cat('Calling read-based CNVs\n')
read.based.cnvs <- mapply(get.read.based.cnvs, cov.cnv.samples, diagnostic.read.counts, MoreArgs = list(2), SIMPLIFY = F)
...
colnames(full.cnv.table) <- stri_replace_all_fixed(unlist(sapply(names(gene.cluster.names), function(x) paste(gene.cluster.names[x], colnames(read.based.cnvs[[x]]), sep = '_'))),
                                                   c('Cyp6mz_Dupm', 'Cyp6mz_Dupz'),
...
cov.based.negatives <- mapply(setdiff, read.cnv.samples, cov.cnv.samples)
# And vice-versa
read.based.negatives <- mapply(setdiff, cov.cnv.samples, read.cnv.samples)

dir.create('target_regions_analysis', showWarnings = FALSE)
write.table(full.cnv.table, file = 'target_regions_analysis/full_CNV_table.csv', sep = '\t', col.names = NA, quote = F)
