...
for al in alignments:
	...
	if (cigar[-1][0] == 4) & (cigar[-1][1] >= 10):
		...
		post_clipping_fastq_file.write(al.seq[(-cigar[-1][1]):] + '\n')
		post_clipping_fastq_file.write('+\n')
		post_clipping_fastq_file.write(al.qual[(-cigar[-1][1]):] + '\n')

	# We check the mapq value. 
	if al.mapq >= float(minqual):
		count += 1
		if soft_clipping_start_point:
			soft_clipping_start_points += [[soft_clipping_start_point, al.seq[(-cigar[-1][1]):]]]
		...
		if al.mapq == 0:
			count_mapq0 += 1
			if soft_clipping_start_point:
				soft_clipping_start_points_mapq0 += [[soft_clipping_start_point, al.seq[(-cigar[-1][1]):]]]
			if soft_clipping_end_point:
				soft_clipping_end_points_mapq0 += [[soft_clipping_end_point, al.seq[:cigar[0][1]]]]
		else:
			count_belowT += 1
			if soft_clipping_start_point:
				soft_clipping_start_points_belowT += [[soft_clipping_start_point, al.seq[(-cigar[-1][1]):]]]
			if soft_clipping_end_point:
