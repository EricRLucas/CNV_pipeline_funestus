...
# Write a function to find the closest value to x from a vector Y
def take_closest(x, Y):
	return min(Y, key = lambda k: abs(k-x))
...
def normalise_coverage_by_GC(coverage, median_coverage_by_GC, ploidy=2):
	...
	# If we come across a GC bin that is not in our table of expectations (median_coverage_by_GC), then
	# we take the nearest one
	output['expcov'] = [median_coverage_by_GC.loc[take_closest(x, median_coverage_by_GC.index)] for x in output['GC']]
